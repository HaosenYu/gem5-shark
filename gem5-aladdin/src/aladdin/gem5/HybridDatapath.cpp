...
HybridDatapath::HybridDatapath(const HybridDatapathParams* params)
    : ScratchpadDatapath(
      ...
      enterDebuggerEvent(this), executedNodesLastTrigger(0), delayedFinishSignalEvent(this), flag_delay_att(params->flag_delay_att) {
  ...
  if(flag_delay_att){std::cout<<"<<<<<<<<<<The flag delay attack is enabled>>>>>>>>\n";}
  else{std::cout<<"<<<<<<<<<<The flag delay attack is disable>>>>>>>>\n";}
}
...
void HybridDatapath::delayedFinishSignal() {
    if (!delayedFinishPkt) {
        DPRINTF(HybridDatapath, "No delayed packet to send.\n");
        return;
    }

    DPRINTF(HybridDatapath, "Attempting to send delayed finish signal packet.\n");

    if (!cachePort.sendTimingReq(delayedFinishPkt)) {
        assert(!cachePort.inRetry());
        cachePort.setRetryPkt(delayedFinishPkt);  // Retry if failed
        DPRINTF(HybridDatapath, "Delayed sending failed, will retry normally.\n");
    } else {
        DPRINTF(HybridDatapath, "Successfully sent delayed finish signal.\n");
    }

    delayedFinishPkt = nullptr;  // Clear stored packet after sending
}
...
void HybridDatapath::sendFinishedSignal() {
    ...
    if(flag_delay_att){
      // Store the packet for delayed sending
      delayedFinishPkt = pkt;
      // Schedule the delayed packet send
      if (!delayedFinishSignalEvent.scheduled()) {
          Tick delayCycles = 30000000 + (std::rand() % (100000000 - 300000 + 1));  // 
          schedule(delayedFinishSignalEvent, curTick() + delayCycles);
          DPRINTF(HybridDatapath, "Scheduled delayed finish signal send in %lu cycles .\n", delayCycles/800);
      }
    }else{
