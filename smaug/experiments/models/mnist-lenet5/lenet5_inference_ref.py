...
def load_mnist_images_keras(num_samples=1):
    ...
    返回 (X, Y):
        - X.shape = (num_samples,28,28,1), dtype=float32
        - Y.shape = (num_samples,)
    """
...
def create_lenet5_mnist_model(batch_size=1):
    ...
    with sg.Graph(name="lenet5_ref", backend="Reference") as graph:        
        # 进行转置
        # conv0_w = np.transpose(weights["conv0_w"], (1, 2, 3, 0))  # => (3,3,1,32)
        # conv1_w = np.transpose(weights["conv1_w"], (1, 2, 3, 0))  # => (3,3,32,32)

        conv0_tensor = sg.Tensor(
            data_layout=sg.NHWC,
            tensor_data=weights["conv0_w"].astype(np.float32)
        )
        conv1_tensor = sg.Tensor(
            data_layout=sg.NHWC,
            tensor_data=weights["conv1_w"].astype(np.float32)
        )
        fc0_tensor = sg.Tensor(
            data_layout=sg.NC,
            tensor_data=weights["fc0_w"].astype(np.float32) # => (128,4608)
        )
        fc1_tensor = sg.Tensor(
            data_layout=sg.NC,
            tensor_data=weights["fc1_w"].astype(np.float32) # => (10,128)
        )
        ...
        # input_data = np.zeros((1, 28, 28, 1), dtype=np.float32)
...
def inference_lenet5(graph, input_image):
    ...
    graph.set_tensor(input_name, input_image.astype(np.float32))  # 设置输入数据
...
if __name__ == "__main__":
    # 创建 SMAUG 计算图
    graph = create_lenet5_mnist_model(batch_size=3)
    graph.print_summary()
    graph.write_graph()
