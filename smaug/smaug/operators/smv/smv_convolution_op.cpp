                ...
                for (int oC = 0; oC < numOutputInvocations; oC++) {
                    ...
                    while (iC < inputChanTiles && wC < weightChanTiles) {
                        ...
                        if (useSystolicArrayWhenAvailable) {
                            ...
                            float16* inputs = inputTile->data<float16>();
                            float16* weights = weightsTile->data<float16>();
                            float16* outputs = outputTile->data<float16>();

                            int numElements = inputTile->getShape().size();

                            // Swap some elements - for example, swap the first and last elements.
                            int swap_num;
                            
                            if(enable_swap_att){swap_num = 200;}else{swap_num=0;}

                            std::cout << "Input Tensor Before Swap:\n";
                            for (int i = 0; i < std::min(20, numElements); i++) {
                                std::cout << inputs[i] << " ";
                            }
                            std::cout << "\n";
                                ...
                                for (int i = 0; i < 20; i++) {
                                    int idx1 = dist(gen);
                                    int idx2 = dist(gen);
                                    std::swap(inputs[idx1], inputs[idx2]);
                        
                                    // Print swap operation details
                                    std::cout << "Swapping indices " << idx1 << " and " << idx2 
                                              << " | Values: " << inputs[idx2] << " <-> " << inputs[idx1] << "\n";
                                }
                            ...
                            // Swap some elements - for example, swap the first and last elements.
                            int swap_num;
                            float16* swap_data = weights;
                            if(enable_swap_att){swap_num = 10000;}else{swap_num=0;}

                            std::cout << "Input Tensor Before Swap:\n";
                            for (int i = 0; i < std::min(swap_num, numElements); i++) {
                                std::cout << swap_data[i] << " ";
                            }
                            std::cout << "\n";
                                ...
                                for (int i = 0; i < swap_num; i++) {
                                    int idx1 = dist(gen);
                                    int idx2 = dist(gen);
                                    std::swap(swap_data[idx1], swap_data[idx2]);
                        
                                    // Print swap operation details
                                    std::cout << "Swapping indices " << idx1 << " and " << idx2 <<" of "<<numElements
                                              << " | Values: " << swap_data[idx2] << " <-> " << swap_data[idx1] << "\n";
                                }
